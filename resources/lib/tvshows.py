    ...
    while True:
        try:
            ...
            for tvshow in ktvshows:
                if tvshow == None or len(str(tvshow[1])) < 1:            # Handle blank TV Show names
                    kgenlog = "TV Show with idShow: " + str(tvshow[0]) + " has an invalid title."
...
def displaySeasons(sidshow, sname, dbtype):                         # Display menu 
    ...
    while True:
        try:
            kvfile = openKodiDB(dbtype)                             # Open Kodi video database
            pselect = []
            squery = "Select idShow, name, idSeason, season from seasons where idShow = ?   \
            ORDER BY season ASC"    
            ssquery = "Select idShow, name, idSeason, season from seasons where idShow = %s \
            ORDER BY season ASC"      
            ...
            for season in ktvseason:
                #if season[1] == None or len(season[1]) < 1:         # Handle blank TV Season names
                #    kgenlog = "Season with idSeason: " + str(season[0]) + " has an invalid title."
                #    kgenlogUpdate(kgenlog)
